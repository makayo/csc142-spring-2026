...
🟡 **Edge Case 1: Submit Form With Empty Fields**
...
Expected Result:
- Publish button remains disabled and no request is sent

Actual Result:
- Button is visually dimmed and unclickable when required fields are empty

✔ Validation prevents incomplete POST requests

---

🟡 **Edge Case 2: Filter by User ID With No Results**

Steps:
- Type **999** in the filter input and click **Go**

Expected Result:
- Empty state renders: "No posts found for User 999"

Actual Result:
- Empty state message displays correctly with no errors

✔ Empty query result is handled gracefully without crashing

---

🟡 **Edge Case 3: Delete With Optimistic Update and Network Failure**

Steps:
- Open DevTools → Network tab → set to Offline
- Click **Delete** on any post and confirm
# Week 4 CRUD Query App (React + Vite + TanStack Query)
...
MARK YOSINAO
Expected Result:
- Post is removed from the UI immediately (optimistic), then restored if the request fails

Actual Result:
- Optimistic removal occurs; on error, `onError` rollback restores the previous cache state

✔ `onMutate` and `onError` work together to maintain data integrity
...
This project demonstrates how TanStack Query manages both server reads and writes in React. Using `useQuery` for fetching and `useMutation` for all write operations, the app maintains a responsive UI through optimistic updates, cache manipulation, and graceful error rollback. The test cases validate both expected user flows and edge conditions including empty states, disabled form submission, and network failure recovery.
