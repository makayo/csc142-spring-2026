type Props = { stepIndex: number; totalSteps: number; };
export default function StatsPanel({ stepIndex, totalSteps }: Props) {
  ...
  return (
    <div className="p-3 bg-slate-800 rounded-lg border border-slate-700 text-sm flex items-center gap-4">
      <span className="text-slate-400">Step <span className="text-white font-bold">{stepIndex}</span> / <span className="text-white font-bold">{totalSteps}</span></span>
